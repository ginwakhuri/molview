...
function clearShownShortestPath(mp) {
    if (
        !mp ||
        !window.Sketcher ||
        !Sketcher.metadata ||
...
function activateTool(mp, $btn, type) {
    jQuery('.tool-button.primary-tool').removeClass('tool-button-selected');
    $btn.addClass('tool-button-selected');
    clearShownShortestPath(mp);
    mp.tool = { type: type, data: {}, selection: [] };
    mp.resetEventDisplay && mp.resetEventDisplay();
    mp.sel.clear();
    mp.requestRedraw();
}

function deactivateToDrag(mp, $btn) {
    $btn.removeClass('tool-button-selected');
    clearShownShortestPath(mp);
    mp.tool = { type: 'drag', data: {} };
    mp.resetEventDisplay && mp.resetEventDisplay();
    mp.sel.clear();
    mp.requestRedraw();
}
...
var Actions = {
	...
	*/
    ...
    export_sketcher_png: function () {
        ...
        if (blob !== null)
            saveAs(blob, document.title + ' (structural formula).png');
    },
    ...
    export_model: function () {
        ...
        saveAs(
            blob,
            document.title + '.' + Model.getFileExstension().toLowerCase()
        );
    },
    ...
    data_spectra: function () {
        ...
        if (
            Spectroscopy.data['smiles'] &&
            Spectroscopy.data['smiles'] === smiles
        ) {
    ...
    search_substructure: function () {
        ...
        Messages.process(function () {
            if (Sketcher.metadata.cid) {
                Loader.PubChem.structureSearch(
                    'cid',
                    Sketcher.metadata.cid,
                    'substructure'
                );
            } else {
                var smiles;
	...
	*/
    ...
    chain_type_ribbon: function () {
        Model.setChainType(
            !$('#action-chain-type-ribbon').hasClass('checked')
                ? 'ribbon'
                : 'none'
        );
    },
    chain_type_cylinders: function () {
        Model.setChainType(
            !$('#action-chain-type-cylinders').hasClass('checked')
                ? 'cylinders'
                : 'none'
        );
    },
    chain_type_btube: function () {
        Model.setChainType(
            !$('#action-chain-type-btube').hasClass('checked')
                ? 'btube'
                : 'none'
        );
    },
    chain_type_ctrace: function () {
        Model.setChainType(
            !$('#action-chain-type-ctrace').hasClass('checked')
                ? 'ctrace'
                : 'none'
        );
    },
	...
	*/
    ...
    jmol_measure_distance: function () {
        ...
        Model.JSmol.setMeasure(
            $('#action-jmol-measure-distance').hasClass('checked')
                ? 'OFF'
                : 'DISTANCE'
        );
    },
    ...
    jmol_measure_angle: function () {
        Messages.alert('measurements_unreliable');
        Model.JSmol.setMeasure(
            $('#action-jmol-measure-angle').hasClass('checked')
                ? 'OFF'
                : 'ANGLE'
        );
    },
    ...
    jmol_measure_torsion: function () {
        Messages.alert('measurements_unreliable');
        Model.JSmol.setMeasure(
            $('#action-jmol-measure-torsion').hasClass('checked')
                ? 'OFF'
                : 'TORSION'
        );
    },
	...
	*/
    ...
    mp_clear: function () {
        if (window.Sketcher && Sketcher.metadata)
            Sketcher.metadata.shortestPath2D = null; // ----- SHPD (Shortest Path by Distances) -----
        Sketcher.clear();
        var el = document.getElementById('shortest-path-info');
    ...
    mp_shortest_path: function () {
        ...
        var isActive =
            $btn.hasClass('tool-button-selected') &&
            mp.tool &&
            mp.tool.type === 'shortest_path';

        if (isActive) {
            deactivateToDrag(mp, $btn);
            return;
        }

        jQuery('#action-mp-shortest-path-bond').removeClass(
            'tool-button-selected'
        );
        activateTool(mp, $btn, 'shortest_path');
    },

    // ----- SHPD (Shortest Path by chemical bond distances) -----
    mp_shortest_path_bond: function () {
        var mp = Sketcher.molpad;
        var $btn = jQuery('#action-mp-shortest-path-bond');
        var isActive =
            $btn.hasClass('tool-button-selected') &&
            mp.tool &&
            mp.tool.type === 'shortest_path_bond';
        ...
        jQuery('#action-mp-shortest-path').removeClass('tool-button-selected');
        activateTool(mp, $btn, 'shortest_path_bond');
    },
	...
	*/
    ...
    export_spectrum_png: function () {
        ...
        var dataURL = document
            .getElementById('spectrum-canvas')
            .toDataURL('image/png');
        var blob = dataURItoBlob(dataURL);
        if (blob !== null)
            saveAs(
                blob,
                $('#spectrum-select').find('option:selected').text() + '.png'
            );
    },
    ...
    export_spectrum_jcamp: function () {
        ...
        if (blob !== null)
            saveAs(
                blob,
                $('#spectrum-select').find('option:selected').text() + '.jdx'
            );
    },
};
