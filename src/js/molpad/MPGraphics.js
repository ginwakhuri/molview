...
MolPad.prototype.update = function () {
    ...
    this.s.atom.scale =
        this.getScale() < this.s.atom.minScale
            ? this.s.atom.minScale / this.getScale()
            : 1;
    this.s.bond.deltaScale =
        this.getScale() < this.s.bond.minDeltaScale
            ? this.s.bond.minDeltaScale / this.getScale()
            : 1;
    this.s.bond.scale =
        this.getScale() < this.s.bond.minScale
            ? this.s.bond.minScale / this.getScale()
            : 1;

    this.s.atom.radiusScaled = this.s.atom.radius * this.s.atom.scale;
    this.s.atom.selectionRadiusScaled =
        this.s.atom.selectionRadius * this.s.atom.scale;
...
MolPad.prototype.setFont = function (type) {
    ...
    var font =
        this.s.fonts[type].fontStyle +
        ' ' +
        Math.round(
            (this.s.fonts[type].fontSize * this.s.atom.scale * 96) / 72
        ) +
        'px ' +
...
MolPad.prototype.redraw = function (update) {
    ...
    if (this.shortestPath && this.shortestPath.totalWeight) {
        this.ctx.save();
        ...
        this.ctx.fillText(
            'Total distance: ' +
                this.shortestPath.totalWeight.toFixed(2) +
                ' Å',
            this.canvas.width / 2, // X coordinate (center)
            30 // Y coordinate (from top)
        );
        this.ctx.restore();
    }
};

// After drawing atoms/bonds/path, near the end of redraw():
if (this.currentShortestPath && this.currentShortestPathDistance != null) {
    var ctx = this.ctx;

    // Compute a label position (here: average of path atom positions)
    var atoms = this.currentShortestPath.atoms || [];
    var x = 0,
        y = 0;
    ...
    if (atoms.length > 0) {
        ...
        x /= atoms.length;
        y /= atoms.length - 0; // average
    } else {
        // Fallback: top-center of canvas
    ...
    ctx.save();
    ctx.font = '14px Arial';
    ctx.fillStyle = '#000'; // or any color you like
    ctx.textAlign = 'center';
    ctx.textBaseline = 'bottom';

    ctx.fillText(
        'Total distance: ' + this.currentShortestPathDistance.toFixed(2),
        x,
        y - 10 // slightly above the atoms
    );
